...
Class autostock_helper {
	...
	private function dataForTable($item,$array = array())
	{
        	$array['id'] = $item->id;
                ...
                $array['sale'] =                number_format(crm_worklist_company::where('wl_id',$item->getWorklistId())->where('razdel',1)->sum('sum'),0,'',' ').' р.';

                $array['price_out_dops'] =      
                        number_format(
                                $item->packPrice()+
                                $item->complect->price-
                                crm_worklist_company::where('wl_id',$item->getWorklistId())->where('razdel',1)->sum('sum'),0,'',' '
                        ).' р.';

                $array['dop_istall'] =          number_format($item->dopprice,0,'',' ').' р.';

                $array['dop_offered'] =         (crm_offered_dop::where('worklist_id',$item->getWorklistId())->first())?number_format(crm_offered_dop::where('worklist_id',$item->getWorklistId())->first()->price,0,'',' ').' р.':'0 р.';

                $array['dop_sale'] =            number_format(crm_worklist_company::where('wl_id',$item->getWorklistId())->where('razdel',2)->sum('sum'),0,'',' ').' р.';

                $array['dop_total_price'] = number_format(
                                $item->dopprice+
                                @crm_offered_dop::where('worklist_id',$item->getWorklistId())->first()->price-
                                crm_worklist_company::where('wl_id',$item->getWorklistId())->where('razdel',2)->sum('sum')
                ,0,'',' ').' р.';  
                ...
                $array['client_pay_sum'] = '0';
                $array['client_pay_date'] = '0';
                $array['client_pay_residue'] = '0';

                /************************************************/
                /*      ПЕРСОНАЛЬНЫЕ ДАННЫЕ КЛИЕНТА НАЧАЛО      */
                /************************************************/
                $client = $item->getClient();//хранит инфо о клиенте что бы по сто раз не вызывать
