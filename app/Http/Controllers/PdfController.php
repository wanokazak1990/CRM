...
class PdfController extends Controller
{
     ...
     */
    public function createOffer(Request $request, $worklist_id = null)
    {
    	if ($worklist_id == null)
        {
            if ($request->has('pdf_cars'))
            {
                foreach ($request->pdf_cars as $key => $car_id) 
                {
        ...
        {
            $car_id = crm_car_selection::where('worklist_id', $worklist_id)->first()->car_id;
            $car = avacar::find($car_id);
            $cars[] = $this->getCarInfo($car);
        }
